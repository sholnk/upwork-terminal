# UpWorkアプリ：FB＋Sophia（統合MD・日本語タイトル）

※キャンバス上の既存ドキュメントはタイトルが長く見分けづらいため、\*\*この統合MDを“正本”\*\*として使ってください。
...
export async function runSophia(args: {
  ...
  const raw = await args.provider.completeJson({ system, user, temperature: 0.2 });
...
## 3-5 Next.js API Route（/api/sophia/analyze）
...
export async function POST(req: Request) {
  ...
  const output = await runSophia({ provider, targetType, targetTitle, targetText, userAnswerJa });
...
export const provider: LLMProvider = {
  async completeJson() {
    return {
      q_meta: {
        intention: "（モック）意図の整理",
        misalignment: "（モック）ズレの可能性",
        latent_frame: "（モック）前提フレーム",
        premise_reflection: "（モック）前提の再確認",
        feedback: "（モック）改善フィードバック",
        socratic_trigger: {
          question_ja: "（モック）次に一つだけ答えてほしい質問は？",
          why_this_question: "（モック）この質問が前に進める理由",
          answer_format_hint: "（モック）箇条書きでOK",
        },
      },
      f_ultimate: {
        awareness: "（モック）現状の気づき",
        classification: "（モック）状況分類",
        navigation: { next_step: "（モック）次の一歩", options: ["A案", "B案"], recommended: "A案" },
        verification: { assumptions: ["前提A"], risks: ["リスクA"], what_to_confirm_next: ["確認A"] },
        redefine: "（モック）再定義",
        meta_check: "（モック）メタチェック",
        r_update: { metrics: ["返信率", "応募数"], cadence: "週次" },
      },
      artifacts: { summary_ja: "（モック）要約" },
    };
  },
};
```
